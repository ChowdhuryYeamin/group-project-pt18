...
int main() {
	...
	for (auto x=begin (list); x != end(list); x++) {
		...
		std::cin >> userinput;
		if(userinput == 1){
			std::cout << "Please enter the name of the file you'd like to process" << endl;
			std::cin >> filename;
			alternateMenu();
			cin >> userinput;
			...
			if(userinput == 1){
				Echo echo;
				echo.Processor8_M_S(unsigned char buffer[], int bufferSize);
			}
			else if(userinput == 2){
				NoiseGate ng;
				ng.Processor8_M_S(unsigned char buffer[], int bufferSize);
			}	
			else if(userinput == 3){
				Limiter lm;
				lm.Processor8_M_S(unsigned char buffer[], int bufferSize);
			}
			else if(userinput == 4){
				Normalization nm;
				nm.Processor8_M_S(unsigned char buffer[], int bufferSize);
			}
			else if(userinput == 5){
				metaDataMenu();
				std::cin >> key;
		...
		// Need to move id3 entry to always be at end of metadata list.  New entries need to be added prior to id3 entry
			...
			//Will add in the stuff for dataprinter
		}
		else{
			continue;
		}
